...
const getters = {
  ignored: state => Object.values(state)
    .reduce((arr, folder) => arr.concat(
      folder.reduce((arr, rh) => arr.concat(rh.ignored), []),
    ), []),

  forFileType: state => (folder, type) => state[folder] && state[folder]
    .filter(rh => rh.fileTypes.indexOf(type) > -1),
};
